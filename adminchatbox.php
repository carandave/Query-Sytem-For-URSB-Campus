        ...
        <div class="modal-content">
             <div class="modal-header">
                ...
                <h4>
                    <?php 
                        ...
                        if(isset($_GET['toId']))
                        {
                            $userName = mysqli_query($conn, "SELECT * FROM users WHERE user_id='".$_GET['toId']."' ");
                            $uName = mysqli_fetch_assoc($userName);

                            echo '<input type="text" value='.$_GET['toId'].' id="toUser" class="d-none"/>';
                            echo 'To: '.$uName['user_name'].' ';
                    ...
                    <input type="text" value="<?php echo $adminId?>" id="fromUser" class="d-none">
             ...
             <div class="modal-body" id="msgBody" style="height: 400px; overflow-y: scroll; overflow-x: hidden;">
                <?php {
                    if(isset($_GET['toId']))
                        {
                            $chats = mysqli_query($conn, "SELECT * FROM conversations WHERE (FromUser='$adminId' AND ToUser='".$_GET['toId']."') OR (FromUser='".$_GET['toId']."' AND ToUser='$adminId')");
                            // $uName = mysqli_fetch_assoc($userName);
                    ...
                    //     while($chat = mysqli_fetch_assoc($chats)){
                    //             if($chat['FromUser'] == $adminId){
                    //             echo "<div style='text-align: right;'>
                    //                     <p style='background: lightblue; word-wrap: break-word; display: inline-block; padding: 5px; border-radius: 10px; max-width: 70px;'>
                    //                     ".$chat["Message"]."
                    //                     </p>
                    //                    </div>";
                    //             }
